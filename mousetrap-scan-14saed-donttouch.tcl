...
analyze -f verilog mousetrap-scan-noreset.v

elaborate top

#set_top_module mousetrap

current_design mousetrap

get_flat_cells

set_dont_use saed14rvt_ss0p6v125c/SAEDRVT14_LDPRSQB_*

# top level is NOT used for ATPG

create_clock -name ph1 -period 10 -waveform {0 3} phi1
create_clock -name ph2 -period 10 -waveform {4 7} phi2
...
get_lib_cells -of_objects L2/q_reg

read_test_protocol mousetrap-scan.stil

dft_drc

#write -hierarchy -format verilog -output MOUSETRAP-SCAN-MAPPED.v
write -hierarchy -format verilog -output MOUSETRAP-SCAN-MAPPED-SAED14-ALTLAT.v
...
-scan_data_out [get_ports test_so] \
-scan_enable [get_ports test_se] \
-scan_master_clock [get_ports phi1] \
-scan_slave_clock [get_ports phi2] \
-infer_dft_signals
